  ...
  return (
    <div className="md:hidden top-5 right-5 fixed w-60 z-[999] flex flex-col items-end gap-2">
      <motion.button
        ...
        whileTap="tap"
        whileHover="hover"
        className=" bg-white w-[3rem] h-[3rem] drop-shadow backdrop-blur-0.5rem] border border-slate-400 dark:border-white border-opacity-400 shadow-2xl rounded-full flex items-center justify-center dark:bg-gray-950">
        ...
        {isOpen && (
          <motion.div
            ...
            {links.map((link, index) => (
              <motion.div className="w-full" key={link.hash}
                ...
                <NextLink className={clsx(
                  ...
                  {
                    "text-gray-950 bg-slate-200 dark:text-gray-200 dark:bg-gray-700 rounded": activeSection === link.hash, "rounded-t-xl round": index === 0, "rounded-b-xl round": index === links.length - 1,
                  }
                )}
                  href={link.hash}
                  onClick={() => {
